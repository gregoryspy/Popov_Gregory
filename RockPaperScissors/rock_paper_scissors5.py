...
class RockPaperScissorsGame:
    ...
    def is_player_win_or_draw(self, computer_move, player_move):
        idx_c = self.options.index(computer_move)
        idx_p = self.options.index(player_move)
        if self.options == OPTIONS:
            ...
            if idx_c == idx_p:
                ...
                if idx_c > idx_p:
                    if (idx_c - idx_p) < len(self.options) / 2:
                        return False
                    else:
                        return True
                else:
                    if (len(self.options) - idx_p + idx_c) < len(self.options) / 2:
                        return False
                    else:
                        return True
