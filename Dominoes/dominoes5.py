...
if __name__ == '__main__':
    ...
    while True:
        ...
        if status == STATUS_COMP:
            ...
            adict = {}
            for k in range(7):
                adict[k] = sum(p.count(k) for p in computer_pieces)
            computer_pieces.sort(key=lambda x: adict[x[0]] + adict[x[1]], reverse=True)
            for piece in computer_pieces:
                if snake[0][0] in piece:
                    piece_move = computer_pieces.pop(computer_pieces.index(piece))
                    if piece_move[0] == snake[0][0]:
                        piece_move = tuple(reversed(piece_move))
                    snake = [piece_move] + snake
                    break
                if snake[-1][-1] in piece:
                    piece_move = computer_pieces.pop(computer_pieces.index(piece))
                ...
                try:
                    ...
                    if move not in range(-len(player_pieces), len(player_pieces)+1):
                        ...
                        if move == 0:
                            ...
                            piece_move = player_pieces[(abs(move) - 1)]
                            if move < 0:
                                if snake[0][0] in piece_move:
                                    piece_move = player_pieces.pop(player_pieces.index(piece_move))
                                    if piece_move[0] == snake[0][0]:
                                        piece_move = tuple(reversed(piece_move))
                                    snake = [piece_move] + snake
                                    break
